// /api/curso-checkout.js
//
// Proxy Vercel Edge → Supabase Edge Function `curso-checkout`.
...
// Sin clase_id — el curso es único, comprable 24/7.
...
export default async function handler(request) {
  ...
  if (!SUPABASE_ANON_KEY) {
    console.error('curso-checkout: SUPABASE_ANON_KEY missing');
  ...
  try {
    const upstream = await fetch(`${SUPABASE_URL}/functions/v1/curso-checkout`, {
      method: 'POST',
      headers: {
    ...
    console.error('curso-checkout proxy error:', err);
